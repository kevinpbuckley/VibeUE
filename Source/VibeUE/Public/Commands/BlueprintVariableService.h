...
/**
 * Main service for blueprint variable management operations
 * Coordinates sub-services and provides high-level variable operations
 */
class VIBEUE_API FBlueprintVariableService : public FServiceBase
{
public:
    explicit FBlueprintVariableService(TSharedPtr<FServiceContext> Context);
    ~FBlueprintVariableService();
    
    // Lifecycle - override from FServiceBase
    virtual void Initialize() override;
    virtual void Shutdown() override;

    // Command routing (for backward compatibility with existing command handler)
...
private:
    ...
    // Static instance
    static TUniquePtr<FBlueprintVariableService> Instance;
    
public:
    static FBlueprintVariableService& Get();
};