...
public:
    ...
    struct FPinOperationResult
    {
        ...
        bool bSuccess;
        FString Message;
        FString Status; // "applied", "noop", "failed", "ignored"
    };
    
    /**
     * Reset multiple pins to their autogenerated default values in a single transaction
     * @param Blueprint Blueprint containing the node
     * @param NodeId GUID or identifier of the node
     * @param PinNames Array of pin names to reset
     * @return Array of per-pin operation results with status
     */
    TResult<TArray<FPinOperationResult>> ResetPinDefaultsBatch(UBlueprint* Blueprint, const FString& NodeId, const TArray<FString>& PinNames);
