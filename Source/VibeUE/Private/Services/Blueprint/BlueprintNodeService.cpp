...
namespace
{
    ...
    FString DescribeContainerType(EPinContainerType Container)
    {
        switch (Container)
        {
    ...
    TSharedPtr<FJsonValue> BuildDefaultValueJson(const UEdGraphPin* Pin)
    {
        ...
        {
            const FString& AutoDefault = Pin->AutogeneratedDefaultValue;
            if (AutoDefault.IsEmpty())
            {
                return MakeShared<FJsonValueNull>();
            }
            return MakeShared<FJsonValueString>(AutoDefault);
        }

        const FName& Category = Pin->PinType.PinCategory;
        if (Category == UEdGraphSchema_K2::PC_Boolean)
        {
            bool bValue = DefaultValue.ToBool();
            return MakeShared<FJsonValueBoolean>(bValue);
        }
        else if (Category == UEdGraphSchema_K2::PC_Int || Category == UEdGraphSchema_K2::PC_Byte)
        {
            int32 IntValue = 0;
            FDefaultValueHelper::ParseInt(DefaultValue, IntValue);
            return MakeShared<FJsonValueNumber>(IntValue);
        }
        else if (Category == UEdGraphSchema_K2::PC_Real || Category == UEdGraphSchema_K2::PC_Double)
        {
            double DoubleValue = 0.0;
            FDefaultValueHelper::ParseDouble(DefaultValue, DoubleValue);
            return MakeShared<FJsonValueNumber>(DoubleValue);
        }

        return MakeShared<FJsonValueString>(DefaultValue);
    }
